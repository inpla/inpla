...
#### Bug fix:
* **Bytecode generation**: The instruction `EQI src fixint dest` was generated as `EQI src int dest`. It was fixed.
* **Bytecode optimisation**:  Blocks for scopes of Copy Propagation optimisation were not specified for each guard expression. It was fixed to work well.
