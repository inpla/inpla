...
VALUE myalloc_Agent(Heap *hp) {
  ...
  return Expand_heap_alloc_Agent(hp);

#elif defined(FLEX_EXPANDABLE_HEAP)
  return Flex_expand_heap_alloc_Agent(hp, Hoop_increasing_magnitude);

#else
  return Heap_alloc_Agent(hp);
...
#ifdef EXPANDABLE_HEAP
  return Expand_heap_alloc_Name(hp);

#elif defined(FLEX_EXPANDABLE_HEAP)
  return Flex_expand_heap_alloc_Name(hp, Hoop_increasing_magnitude);

#else
  return Heap_alloc_Name(hp);
...
void VM_Buffer_Init(VirtualMachine * restrict vm, int size) {
...
void VM_Init(VirtualMachine * restrict vm, 
  ...
  VM_Buffer_Init(vm, agentBufferSize);
  VM_EQStack_Init(vm, eqStackSize);
}
#endif
